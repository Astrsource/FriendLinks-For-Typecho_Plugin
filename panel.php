...
<script>
    ...
    function submitAjax(form, callback) {
        ...
        var action = data.get('action');  // 获取操作类型
    
        // 如果是“更新所有链接信息”，不等待完整响应，只提交任务
        if (action === 'update_all') {
            if (!confirm('<?php _e('确定要更新所有链接的信息吗？这将耗时较长，建议使用 Cron 定时任务。'); ?>')) {
                return;
            }
            if (btn) { btn.disabled = true; btn.textContent = '提交中...'; }
            fetch(window.location.href, { method: 'POST', body: data })
                .then(() => {
                    alert('<?php _e('更新任务已启动，请稍后手动刷新页面查看结果。'); ?>');
                })
                .catch(e => alert('<?php _e('请求失败：'); ?>' + e))
                .finally(() => {
                    if (btn) { btn.disabled = false; btn.textContent = originalText; }
                });
            return;  // 不执行后续的 location.reload()
        }
    
        // 其他操作保持原有逻辑：等待完成后刷新页面
